...
static void transpose_blocks(omatrix_t *m,
		unsigned int blocki, unsigned int blockj,
		unsigned int bsize)
{
	matrix_elem_t *ij, *ji, tmp;
	unsigned int i, j;
	for(i = blocki;i < (blocki+ bsize);i++){
		for(j = blockj;j<(blockj + bsize);j++){
			ij = MATRIX_ELEMP(m, i, j);
			tmp = *ij;
			/* i, j = j, i */
			ji = MATRIX_ELEMP(m, j, i);
...
void omatrix_transpose(omatrix_t *m, unsigned int bsize)
{
	...
	/* Non-diagonal blocks */
	for(i = 0; i < (m->d); i+=bsize){
		for(j = i+bsize;j<(m->d); j+=bsize){
			transpose_blocks(m,i,j,bsize);
		}
	}

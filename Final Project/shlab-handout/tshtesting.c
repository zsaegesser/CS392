...
Search
Sign in or Sign up
 Watch 1  Star 0  Fork 0 ss250294/tiny-shell-assignment
 Code  Issues 0  Pull requests 0  Projects 0  Pulse  Graphs
Branch: master Find file Copy pathtiny-shell-assignment/tsh.c
a18ab98  on May 5, 2013
 ss250294 File added
0 contributors
RawBlameHistory
705 lines (589 sloc)  17.1 KB
/*
 * tsh - A tiny shell program with job control
 *
 * <Shubham Srivastava 201101101>
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
void eval(char *cmdline)
{
    ...
    /* Block signals until we can add the job to the job list. So we won't have
     * problems receiving SIGCHLD, SIGINT, and SIGTSTP signals before adding
     * the job.
     */

    /* sigemptyset, sigaddset, sigprocmask return 0 on success and -1 on error */
    if(sigemptyset(&mask)) unix_error("eval(): sigemptyset() error");
    if(sigaddset(&mask,SIGCHLD)) unix_error("eval(): sigaddset() error");
    if(sigaddset(&mask,SIGINT)) unix_error("eval(): sigaddset() error");
	...
	{
            /* New jobs must get new process ID for
	     * ctrl-c and ctrl-z to work correctly.
	     * (i.e. not kill tsh!!)
	     */
	...
	/* If !bg evaluates high then parent waits for foreground job
	 * to terminate. Otherwise the job is pushed into the background
	 * and process information is displayed.
	 */
	if(!bg) waitfg(pid);
...
void waitfg(pid_t pid)
{
    ...
    while(fg_job->pid == pid && fg_job->state == FG) sleep(1);
...
void sigint_handler(int sig)
{
    ...
    if(verbose) printf("sigint_handler(): begin\n");
    ...
    if(verbose) printf("sigint_handler(): end\n");
...
void sigtstp_handler(int sig)
{
    ...
    if(verbose) printf("sigtstp_handler(): begin\n");
    ...
    if(verbose) printf("sigstp_handler(): end\n");
